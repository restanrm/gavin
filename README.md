...
- `GET /api/vinyls?search=&metadata=missing&genre=&sort=` - Get all vinyls (with optional search, missing-metadata filter, genre filter, and sorting by `artist`, `date`, `last_edit`, or `genre`)
